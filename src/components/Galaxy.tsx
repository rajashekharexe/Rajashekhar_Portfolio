...
void main(){
  ...
  if(uAutoCenterRepulsion>0.0){
    ...
    uv+=normalize(uv-mp)*(uRepulsionStrength/(md+0.1))*0.05*uMouseActiveFactor;
  } else {
...
export default function Galaxy({
  ...
  const isVisible    = useRef(false)   // ← pause when off-screen
  ...
  useEffect(() => {
    ...
    function resize() {
      // Render at native CSS pixels (DPR=1) — 4× cheaper on retina, visually fine
    ...
    function update(t: number) {
      ...
      // ── Skip GPU work when element is off-screen ──────────────────────
      ...
      const lf = 0.06
      smoothMouse.current.x  += (targetMouse.current.x  - smoothMouse.current.x)  * lf
      smoothMouse.current.y  += (targetMouse.current.y  - smoothMouse.current.y)  * lf
      smoothActive.current   += (targetActive.current   - smoothActive.current)   * lf
    ...
    function onMove(e: MouseEvent) {
      ...
      targetMouse.current  = { x: (e.clientX - r.left) / r.width, y: 1 - (e.clientY - r.top) / r.height }
      targetActive.current = 1.0
    }
    function onLeave() { targetActive.current = 0.0 }
